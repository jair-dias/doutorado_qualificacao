...
param_grid = [
  {'C': [0.01, 0.025, 0.05, 0.1, 0.5, 1, 5, 10, 50, 100, 500, 1000], 'kernel': ['linear'],'shrinking':[True, False], 'class_weight':['balanced', None]},
  {'C': [0.01, 0.025, 0.05, 0.1, 0.5, 1, 5, 10, 50, 100, 500, 1000], 'gamma': [0.0001, 0.001, 0.01, 0.1, 1, 2, 5, 10, 'auto', 'scale'],
   'shrinking':[True, False], 'kernel': ['rbf'],'class_weight':['balanced', None]},
 ]
grid_search_svc = GridSearchCV(SVC(),param_grid,refit = True, verbose=2, cv=5, scoring='accuracy', return_train_score=True)
...
means_train = grid_result_svc.cv_results_['mean_train_score']
stds_train = grid_result_svc.cv_results_['std_train_score']
...
y_predicted_svc = grid_search_svc.predict(X_test)
...
# Salvar resultaos em planilha

# Converter o dicionário de resultados para um DataFrame do pandas
results_df = pd.DataFrame(grid_result_svc.cv_results_)
...
output_filename = "grid_search_svm_results.csv"
...
# [[10  6]
#  [ 2  9]]
# Best: 0.934615 using {'C': 0.5, 'class_weight': 'balanced', 'gamma': 'scale', 'kernel': 'rbf', 'shrinking': True}
