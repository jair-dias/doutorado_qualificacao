...
print("training data score: " + str(rf.score(X_train, y_train)))
print("test data score: " + str(rf.score(X_test, y_test)))
...
grid_search_rf = GridSearchCV(estimator=rf, param_grid=param_grid, cv=5, verbose=2, scoring='accuracy',return_train_score=True)
grid_result_rf = grid_search_rf.fit(X_train, y_train)

# Random Forest summarize results
...
results_df = pd.DataFrame(grid_result_rf.cv_results_)
...
output_filename = "grid_search_random_forest_results.csv"
...
# Best: 0.951282 using {'ccp_alpha': 0.1, 'class_weight': None, 'criterion': 'gini', 'max_depth': 50, 'max_features': 'log2', 'min_samples_leaf': 0.01, 'min_samples_split': 0.01, 'min_weight_fraction_leaf': 0.1, 'n_estimators': 100}